...
Nếu đã quen cách giao việc trọn gói ở [bài setup một lần để agent tự chạy](../../08-chuan-hoa-du-an/06-setup-mot-lan-de-agent-tu-chay.md) từ dự án 02, áp dụng lại tinh thần đó ở đây: mỗi nhóm task lớn (chunking, embedding, API hỏi đáp) một nhánh riêng, tự chạy thử trên máy mình trước khi gộp - đặc biệt đáng làm khi tài liệu test là dữ liệu nội bộ nhạy cảm. Lưu ý: bài 06 dùng ví dụ Vercel nên có "bản xem thử" tự động theo nhánh; dự án này deploy VPS nên bạn tự kiểm tra bằng cách chuyển sang đúng nhánh đó trên máy mình (`git switch ten-nhanh`) trước khi gộp, thay vì có sẵn một link xem thử riêng.

...
| **Port (cổng)** | "Cửa" một chương trình đang lắng nghe trên một máy - một máy chạy nhiều chương trình cùng lúc thì mỗi chương trình một port riêng để không giẫm lên nhau (ví dụ `localhost:3000` là máy bạn, cổng 3000). Xem [`01-bat-dau-tu-so-0/10-file-web-va-server-hoat-dong-the-nao.md`](../01-bat-dau-tu-so-0/10-file-web-va-server-hoat-dong-the-nao.md) |
...
| **REST / RESTful API** | Quy ước phổ biến để thiết kế API: mỗi loại dữ liệu có một địa chỉ (URL) riêng, hành động (đọc/tạo/sửa/xóa) thể hiện qua HTTP method (GET/POST/PATCH/DELETE) thay vì nhét vào tên đường dẫn. Xem [`01-bat-dau-tu-so-0/11-json-api-database-crud.md`](../01-bat-dau-tu-so-0/11-json-api-database-crud.md) |
...
| **Status code (mã trạng thái HTTP)** | Con số server trả về kèm mỗi response, cho biết request thành công hay lỗi loại gì - phổ biến nhất: `200` (thành công), `201` (tạo mới thành công), `400` (dữ liệu gửi lên sai), `401`/`403` (chưa đăng nhập/không đủ quyền), `404` (không tìm thấy), `500` (server bị lỗi). Xem [`01-bat-dau-tu-so-0/11-json-api-database-crud.md`](../01-bat-dau-tu-so-0/11-json-api-database-crud.md) |

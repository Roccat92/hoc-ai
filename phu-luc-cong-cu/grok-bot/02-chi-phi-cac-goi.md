...
## Ba lớp tiền thường gặp

Grok Bot gắn hệ Cursor — cách tính tiền **đổi theo thời gian và theo tài khoản**. Luôn đối chiếu màn hình Usage trong app và trang giá chính thức; bảng dưới đây là **khung tư duy**, không phải bảng niêm yết cố định.

| Lớp | Thường là gì | Bạn thấy ở đâu |
|---|---|---|
| **Gói / plan Cursor** | Thuê bao tháng (hoặc trial) mở quyền dùng agent, model, hạn mức kèm | Settings → **Usage & Billing** → plan |
...
Nếu không thấy tab: tài khoản có thể chưa bật billing trên bản build đó — dùng trang tài khoản Cursor trên web hoặc hỏi đúng kênh hỗ trợ sản phẩm, đừng đoán giá từ bài blog cũ.

## Việc gì “đốt” hạn mức nhanh

| Việc | Vì sao tốn |
|---|---|
| Agent vòng lặp dài (cài Unity, resolve package, rebuild liên tục) | Nhiều lượt tool + context dài |
...
**Mẹo thực tế:** giao việc bằng một brief đủ ràng buộc (“chỉ đọc”, “chỉ sửa file A”, “xong thì dừng”), tránh chuỗi “ok / tiếp / xem chưa” nếu chưa có kết quả. Đó là khác biệt lớn so với chat giải trí.
...
| Build sản phẩm gọi API model riêng | Tách: Grok Bot để vận hành máy; API LLM riêng cho app — xem [gọi API LLM](../../04-build-ung-dung-ai/01-goi-api-llm.md) |

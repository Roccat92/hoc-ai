...
Trang này gom chi tiết đặc thù của **Grok Bot** (app agent), không phải cấu hình API **Grok AI**. Đọc khi bạn đã chạy được task cơ bản ở [bài cài đặt](01-cai-dat-va-lenh-co-ban.md).

## Computer và máy local

- **Computer của Bot:** nơi agent cài tool, lưu scratch, giữ session trình duyệt. Các agent của cùng user **chia sẻ** filesystem/login máy này nhưng **không** chia sẻ màn hình desktop từng agent.
...
Grok Bot nối dịch vụ ngoài (GitHub, lịch, chat...) qua **connector / MCP / plugin** trong app - không phải file `.claude/settings.json` kiểu Claude Code, cũng không phải chỉ "bật browsing" trong chat Grok AI.
...
Chat Grok AI thông thường **không** có routine kiểu này; đó là điểm khác rõ giữa "hỏi model" và "có Bot chạy giúp".
...
Khái niệm subagent / tự động hóa ở lộ trình: [subagent và tự động hóa](../../02-code-voi-ai/16-codex-subagent-va-tu-dong-hoa.md) - Grok Bot có worker nền trong phiên; bạn giao việc ở mức mục tiêu, không cần nhắc tên machinery nội bộ.
